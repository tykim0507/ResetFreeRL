        ...
        for episode_idx in tqdm.tqdm(range(args.num_trials_per_task)):
            ...
            while t < max_steps + args.num_steps_wait:
                try:
                    ...
                    if done:
                        # Reset only the robot to initial pose, keeping objects where they are
                        # current_state = env.env.sim.get_state()
                        # # Replace only robot qpos and qvel with initial values
                        # current_state.qpos[:9] = initial_robot_qpos
                        # current_state.qvel[:9] = initial_robot_qvel
                        # env.env.sim.set_state(current_state)
                        # env.env.sim.forward()  # Forward the simulation to update derived quantities
                        
                        # # Update observations after robot reset
                        ...
                        while True:
                            # reverse the process
                            reversed_task_description = "pick up the black bowl"
                            img = np.ascontiguousarray(obs["agentview_image"][::-1, ::-1])
                            wrist_img = np.ascontiguousarray(obs["robot0_eye_in_hand_image"][::-1, ::-1])
                            img = image_tools.convert_to_uint8(
                                image_tools.resize_with_pad(img, args.resize_size, args.resize_size)
                            ...
                            if not action_plan:
                                ...
                                element = {
                                    ...
                                    "prompt": reversed_task_description,
                                }
                            ...
                            # Execute action in environment
                            env.env.render()
                            obs, reward, _, info = env.step(action.tolist())
                        task_successes += 1
                        total_successes += 1
                        break
                    t += 1
